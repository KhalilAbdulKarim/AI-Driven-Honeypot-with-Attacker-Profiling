...
class SessionLogger:
    def __init__(self, session_id: str, client_ip: str, client_port: int):
        ...
        # log connection itself so session is never empty — ensures profiling
        self.log_auth("unknown", "connection", success=False)
    ...
    def log_command(self, command: str, response: str):
        ...
        db.insert_command(
            self.session_id, command.strip(), response.strip(), offset, ts
        )
        self._save()
